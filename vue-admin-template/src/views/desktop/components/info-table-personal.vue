    ...
    <table class="info-table">
      ...
      <tr>
        ...
        <td class="value" colspan="7">
          <span class="red" v-if="taskInfo.dangerLevel">{{
            taskInfo.dangerLevel
          }}</span>
          {{ taskInfo.complaints }}
        </td>
      </tr>
      ...
      <template v-for="(item, index) in dispatchRecordList">
        ...
        <tr :key="index + 'two'">
          ...
          <td class="value">{{ item.nodeName }}</td>
          <td class="key">送往医院：</td>
          <td class="value">{{ item.eventLocation }}</td>
        </tr>
      </template>
    </table>
